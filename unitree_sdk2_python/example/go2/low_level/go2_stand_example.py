...
class Custom:
    def __init__(self):
        ...
        self.duration_4 = 900
        self.duration_5 = 1000
        self.duration_6 = 1000
        self.percent_1 = 0
        self.percent_2 = 0
        self.percent_3 = 0
        self.percent_4 = 0
    ...
    def LowStateMessageHandler(self, msg: LowState_):
        ...
        q_0 = msg.motor_state[go2.LegID["FR_1"]].q
        dq_0 = msg.motor_state[go2.LegID["FR_1"]].dq
        tau_0 = msg.motor_state[go2.LegID["FR_1"]].tau_est
        sys.stdout.write(f"\rFR_0 | q: {q_0:.4f}" ) # rad | dq: {dq_0:.4f} rad/s | tau: {tau_0:.4f} Nm")
        sys.stdout.flush()
    ...
    def LowCmdWrite(self):
        ...
        if (self.percent_1 == 1) and (self.percent_2 == 1) and (self.percent_3 == 1) and (self.percent_4 == 1) and (self.percent_5 <= 1):
            self.percent_5 += 1.0 / self.duration_5
            self.percent_5 = min(self.percent_5, 1)
            for i in range(12):
                self.low_cmd.motor_cmd[i].q = (1 - self.percent_5) * self._targetPos_3[i] + self.percent_5 * self._targetPos_2[i]
                self.low_cmd.motor_cmd[i].dq = 0
                self.low_cmd.motor_cmd[i].kp = self.Kp
                self.low_cmd.motor_cmd[i].kd = self.Kd
                self.low_cmd.motor_cmd[i].tau = 0
        
        if (self.percent_1 == 1) and (self.percent_2 == 1) and (self.percent_3 == 1) and (self.percent_4 == 1) and (self.percent_5 == 1) and (self.percent_6 <= 1):
            self.percent_6 += 1.0 / self.duration_6
            self.percent_6 = min(self.percent_6, 1)
            for i in range(12):
                self.low_cmd.motor_cmd[i].q = (1 - self.percent_6) * self._targetPos_2[i] + self.percent_6 * self._targetPos_1[i]
                self.low_cmd.motor_cmd[i].dq = 0
                self.low_cmd.motor_cmd[i].kp = self.Kp
                self.low_cmd.motor_cmd[i].kd = self.Kd
                self.low_cmd.motor_cmd[i].tau = 0
