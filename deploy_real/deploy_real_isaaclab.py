...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        # [Etape] 1. Charger la la politique 
        print("Etape 1. Chargement de la politique ")
        self.policy = torch.jit.load("/home/theo/deploy/pre_train/go2/policy.pt")
        ...
        self._targetPos_3 = [-0.1,  0.8, -1.5, 0.1, 0.8, -1.5, -0.1, 1, -1.5, 0.1, 1, -1.5]
        ...
        # [Etape] 2. Initialiser les Channels
        print("Etape 2. Initialisation des Channels")
    ...
    def run(self):
        ...
        for i in range(len(self.config.leg_joint2motor_idx)):
            ...
            self.low_cmd.motor_cmd[motor_idx].q = target_dof_pos[i]#self.config.default_angles[i]
            self.low_cmd.motor_cmd[motor_idx].qd = 0
            self.low_cmd.motor_cmd[motor_idx].kp =  25 # normalement: self.config.kps[i]
            self.low_cmd.motor_cmd[motor_idx].kd = 0.5 # normalement: self.config.kds[i]
            self.low_cmd.motor_cmd[motor_idx].tau = 0
        self.send_cmd(self.low_cmd)

        """
        a = round(self.obs[3],1)
        b = round(self.obs[4],1)
        c = round(self.obs[5],1)
        print(f'\r{a,b,c}', end='', flush=True)
        """
        
        """
        # Visualiser les qj_obs
        a = round(self.obs[12],2)
        b = round(self.obs[13],2)
        c = round(self.obs[14],2)
        d = round(self.obs[15],2)
        e = round(self.obs[16],2)
        f = round(self.obs[17],2)
        g = round(self.obs[18],2)
        h = round(self.obs[19],2)
        i = round(self.obs[20],2)
        j = round(self.obs[21],2)
        k = round(self.obs[22],2)
        l = round(self.obs[23],2)
        print(f'\r{a,b,c,d,e,f,g,h,i,j,k,l}', end='', flush=True)
        """

        """
        # Visualiser les dqj_obs
        a = round(self.obs[24],2)
        b = round(self.obs[25],2)
        c = round(self.obs[26],2)
        d = round(self.obs[27],2)
        e = round(self.obs[28],2)
        f = round(self.obs[29],2)
        g = round(self.obs[30],2)
        h = round(self.obs[31],2)
        i = round(self.obs[32],2)
        j = round(self.obs[33],2)
        k = round(self.obs[34],2)
        l = round(self.obs[35],2)
        print(f'\r{a,b,c,d,e,f,g,h,i,j,k,l}', end='', flush=True)
        """
        """
        # Visualiser les actions
        a = round(self.obs[36],2)
        b = round(self.obs[37],2)
        c = round(self.obs[38],2)
        d = round(self.obs[39],2)
        e = round(self.obs[40],2)
        f = round(self.obs[41],2)
        g = round(self.obs[42],2)
        h = round(self.obs[43],2)
        i = round(self.obs[44],2)
        j = round(self.obs[45],2)
        k = round(self.obs[46],2)
        l = round(self.obs[47],2)
        print(f'\r{a,b,c,d,e,f,g,h,i,j,k,l}', end='', flush=True)
        """
        """
        # Visualiser les angles target
        a = round(target_dof_pos[0],2)
        b = round(target_dof_pos[1],2)
        c = round(target_dof_pos[2],2)
        d = round(target_dof_pos[3],2)
        e = round(target_dof_pos[4],2)
        f = round(target_dof_pos[5],2)
        g = round(target_dof_pos[6],2)
        h = round(target_dof_pos[7],2)
        i = round(target_dof_pos[8],2)
        j = round(target_dof_pos[9],2)
        k = round(target_dof_pos[10],2)
        l = round(target_dof_pos[11],2)
        print(f'\r{a,b,c,d,e,f,g,h,i,j,k,l}', end='', flush=True)
        """
        # Visualiser le couple
        kp=20
        kd=5
        a = round(kp*(target_dof_pos[0] - self.obs[12]) - kd*self.obs[24],2)
        b = round(kp*(target_dof_pos[1] - self.obs[13]) - kd*self.obs[25],2)
        c = round(kp*(target_dof_pos[2] - self.obs[14]) - kd*self.obs[26],2)
        d = round(kp*(target_dof_pos[3] - self.obs[15]) - kd*self.obs[27],2)
        e = round(kp*(target_dof_pos[4] - self.obs[16]) - kd*self.obs[28],2)
        f = round(kp*(target_dof_pos[5] - self.obs[17]) - kd*self.obs[29],2)
        g = round(kp*(target_dof_pos[6] - self.obs[18]) - kd*self.obs[30],2)
        h = round(kp*(target_dof_pos[7] - self.obs[19]) - kd*self.obs[31],2)
        i = round(kp*(target_dof_pos[8] - self.obs[20]) - kd*self.obs[32],2)
        j = round(kp*(target_dof_pos[9] - self.obs[21]) - kd*self.obs[33],2)
        k = round(kp*(target_dof_pos[10] - self.obs[22]) - kd*self.obs[34],2)
        l = round(kp*(target_dof_pos[11] - self.obs[23]) - kd*self.obs[35],2)
        print(f'\r{a,b,c,d,e,f,g,h,i,j,k,l}', end='', flush=True)
